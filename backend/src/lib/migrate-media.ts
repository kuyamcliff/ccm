...
export async function migrateInlineMedia(): Promise<void> {
  let moved = 0;
  ...
  for (const row of galleryRows) {
    ...
    if (path) { await updateGallery.run(path, row.id); moved++; }
  }
  ...
  for (const row of menuRows) {
    const path = convert(row.image_url, false);
    if (path) { await updateMenu.run(path, row.id); moved++; }
  }
  ...
  for (const row of reviewRows) {
    ...
    if (changed) await updateReview.run(JSON.stringify(next), row.id);
  }
