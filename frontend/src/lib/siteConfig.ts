...
export const DEFAULT_SITE_CONFIG: SiteConfig = {
  ...
  business: {
    mode: "open",
    message: {
      en: "We're open and ready for you.",
      fr: "Nous sommes ouverts et prêts à vous accueillir.",
    },
  },
  services: {
    ordering: {
      mode: "open",
      message: {
        en: "Takeaway is paused right now while we catch up.",
        fr: "Les commandes à emporter sont momentanément suspendues pendant que nous rattrapons le retard.",
      },
    },
    booking: {
      mode: "open",
      message: {
        en: "Online bookings are paused right now.",
        fr: "Les réservations en ligne sont momentanément suspendues.",
      },
    },
    waitlist: {
      mode: "open",
      message: {
        en: "The waitlist is paused right now.",
        fr: "La liste d'attente est momentanément suspendue.",
      },
    },
  },
  support: {
    ...
    phone: true,
    afterHoursMessage: {
      en: "Nobody is at the desk right now. Leave a message and we'll get back to you.",
      fr: "Personne n'est disponible pour le moment. Laissez un message et nous vous répondrons.",
    },
  },
  payments: {
    mtn: true,
...
function localized(value: unknown, fallback: LocalizedMessage): LocalizedMessage {
  ...
  return {
    ...
    fr: typeof obj.fr === "string" ? obj.fr : fallback.fr,
  };
}
...
export function parseSiteConfig(raw?: string): SiteConfig {
  ...
  try {
    ...
    const businessMessage = localized(business.message, DEFAULT_SITE_CONFIG.business.message);
    ...
    return {
      ...
      locales: {
        en: bool((parsed.locales as Record<string, unknown> | undefined)?.en, true),
        fr: bool((parsed.locales as Record<string, unknown> | undefined)?.fr, true),
      },
      features: {
      ...
      homepage: {
        hero: bool(homepage.hero, DEFAULT_SITE_CONFIG.homepage.hero),
        featured: bool(homepage.featured, DEFAULT_SITE_CONFIG.homepage.featured),
        ways: bool(homepage.ways, DEFAULT_SITE_CONFIG.homepage.ways),
        accountCta: bool(homepage.accountCta, DEFAULT_SITE_CONFIG.homepage.accountCta),
        reviews: bool(homepage.reviews, DEFAULT_SITE_CONFIG.homepage.reviews),
        location: bool(homepage.location, DEFAULT_SITE_CONFIG.homepage.location),
      },
      business: {
      ...
      services: {
        ordering: {
          mode: mode(ordering.mode, ["open", "paused"] as const, "open"),
          message: localized(ordering.message, DEFAULT_SITE_CONFIG.services.ordering.message),
        },
        booking: {
          mode: mode(booking.mode, ["open", "paused"] as const, "open"),
          message: localized(booking.message, DEFAULT_SITE_CONFIG.services.booking.message),
        },
        waitlist: {
          mode: mode(waitlist.mode, ["open", "paused"] as const, "open"),
          message: localized(waitlist.message, DEFAULT_SITE_CONFIG.services.waitlist.message),
        },
      },
      support: {
        enabled: bool(support.enabled, DEFAULT_SITE_CONFIG.support.enabled),
        staffed: bool(support.staffed, DEFAULT_SITE_CONFIG.support.staffed),
        responseMinutes:
          typeof support.responseMinutes === "number" && Number.isFinite(support.responseMinutes)
            ? Math.max(1, Math.min(1440, Math.round(support.responseMinutes)))
            : DEFAULT_SITE_CONFIG.support.responseMinutes,
        whatsapp: bool(support.whatsapp, DEFAULT_SITE_CONFIG.support.whatsapp),
        phone: bool(support.phone, DEFAULT_SITE_CONFIG.support.phone),
        afterHoursMessage: localized(support.afterHoursMessage, DEFAULT_SITE_CONFIG.support.afterHoursMessage),
      },
      payments: {
        mtn: bool(payments.mtn, DEFAULT_SITE_CONFIG.payments.mtn),
        orange: bool(payments.orange, DEFAULT_SITE_CONFIG.payments.orange),
      },
      announcement: {
        enabled: bool(announcement.enabled, DEFAULT_SITE_CONFIG.announcement.enabled),
