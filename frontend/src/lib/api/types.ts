...
/** `position` and `is_active` are only ever present on the console's own
    fetch (`/api/admin/menu`) — the public menu drops both, since every dish
    the customer-facing endpoint returns is already active and pre-ordered. */
...
export interface DiningTable { id: number; label: string; capacity: number; zone: string; pos_x: number; pos_y: number; active: number; available?: boolean; /** Somebody is sitting at it right now. Only the console cares; the guest's plan ignores it. */ in_use?: boolean; }
export type BookingStatus = "pending_payment" | "confirmed" | "cancelled" | "completed";
export type PaymentState = "unpaid" | "paid" | "refunded";
export interface Booking { id: number; date: string; time: string; party_size: number; phone: string; note: string; status: BookingStatus; payment_status: PaymentState; cancellation_fee_fcfa: number; ccm_code: string | null; cancelled_at: string | null; cancel_reason: string | null; checked_in_at: string | null; /** 1 when the guest said so themselves rather than being checked in at the door. */ arrived_by_guest?: number; table_label?: string | null; table_zone?: string | null; /** Every table the booking holds, comma separated. `table_label` is the lead one. */ table_labels?: string | null; created_at: string; amount_fcfa?: number | null; discount_fcfa?: number | null; pay_method?: string | null; pay_reference?: string | null; items_json?: string | null; items_total_fcfa?: number | null; deposit_fcfa?: number | null; }
...
export interface TakeawayOrder { id: number; order_no: string; name: string; phone: string; items_json: string; total_fcfa: number; pickup_time: string; note: string; status: OrderStatus; promo_code: string | null; gift_card_code: string | null; discount_fcfa: number; payment_status?: PaymentState; collected_at?: string | null; /** 1 when the guest confirmed collection rather than a member of staff. */ collected_by_guest?: number; created_at: string; }
