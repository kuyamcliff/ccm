...
export const deskApi = {
  ...
  audit: (filters: { q?: string; limit?: number; offset?: number } = {}) => http.get<{ entries: AuditEntry[]; more: boolean }>(`/api/admin/audit${http.query(filters)}`),
  ...
  reviews: { list: () => http.get<{ reviews: Review[] }>("/api/reviews").then((r) => r.reviews), remove: (id: number) => http.del<{ ok: true }>(`/api/admin/reviews/${id}`), reply: (id: number, text: string) => http.post<{ ok: true }>(`/api/admin/reviews/${id}/reply`, { text }), removeReply: (id: number) => http.del<{ ok: true }>(`/api/admin/reviews/${id}/reply`) },
  gallery: { all: () => http.get<{ photos: GalleryPhoto[] }>("/api/gallery/all").then((r) => r.photos), pending: () => http.get<{ photos: GalleryPhoto[] }>("/api/gallery/pending").then((r) => r.photos), upload: (imageUrl: string, caption: string) => http.post<{ ok: true; photo: GalleryPhoto }>("/api/gallery/admin-upload", { image_url: imageUrl, caption }), update: (id: number, input: { is_approved?: boolean; is_featured?: boolean }) => http.patch<{ ok: true }>(`/api/gallery/${id}`, input), remove: (id: number) => http.del<{ ok: true }>(`/api/gallery/${id}`) },
  events: { list: () => http.get<{ bookings: EventEnquiry[] }>("/api/events").then((r) => r.bookings), create: (input: { name: string; email: string; phone: string; event_type: EventType; date: string; time: string; guest_count: number; note?: string; status?: string }) => http.post<{ ok: true }>("/api/events/admin-create", input), setStatus: (id: number, status: "pending" | "confirmed" | "cancelled") => http.patch<{ ok: true }>(`/api/events/${id}`, { status }), remove: (id: number) => http.del<{ ok: true }>(`/api/events/${id}`) },
  waitlist: { list: () => http.get<{ entries: WaitlistEntry[] }>("/api/waitlist/all").then((r) => r.entries), setStatus: (id: number, status: string) => http.patch<{ ok: true }>(`/api/waitlist/${id}`, { status }), clear: () => http.del<{ ok: true }>("/api/waitlist/clear") },
  offers: { list: () => http.get<{ offers: Offer[] }>("/api/offers/all").then((r) => r.offers), create: (input: Partial<Offer>) => http.post<{ ok: true }>("/api/offers", input), update: (id: number, input: Partial<Offer>) => http.patch<{ ok: true }>(`/api/offers/${id}`, input), remove: (id: number) => http.del<{ ok: true }>(`/api/offers/${id}`) },
  promos: { list: () => http.get<{ codes: PromoCode[] }>("/api/promos").then((r) => r.codes), create: (input: Partial<PromoCode>) => http.post<{ ok: true }>("/api/promos", input), setActive: (id: number, isActive: boolean) => http.patch<{ ok: true }>(`/api/promos/${id}`, { is_active: isActive }), remove: (id: number) => http.del<{ ok: true }>(`/api/promos/${id}`) },
