...
const FURNITURE_LABELS: Record<FurnitureType, string> = {
  tv: "TV", bar: "Bar", plant: "Plant", sofa: "Lounge", host: "Host", stage: "Stage",
};
...
export function AdminTableEditor() {
    ...
    if (dragging) {
      ...
      if (pos) {
        try {
          ...
          setError("Could not save table position.");
          await load();
        }
      }
    }
  ...
  async function doEditTable() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Could not update table.");
    } finally {
      setEditBusy(false);
    }
  }
  ...
  async function doDelete() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Could not remove table.");
    }
  }
  ...
  return (
    <div>
      <div className="fp-header">
        <div>
          <p className="fp-eyebrow">Admin · Floor plan</p>
          <h1 className="fp-title">Floor plan</h1>
        </div>
        <div className="fp-toolbar">
          {editMode && (
            <>
              <button className="btn btn-outline btn-sm" onClick={() => setAddFurnitureOpen(true)}>+ Furniture</button>
              <button className="btn btn-amber btn-sm" onClick={() => setAddOpen(true)}>+ Table</button>
            </>
          )}
          <button
      ...
      {loading ? (
        <p className="empty-admin">Loading floor plan…</p>
      ) : (
        <div className="fp-body">
          {/* Map column */}
          <div className="fp-map-col">
            <div className="fp-legend">
              <span className="fp-leg fp-leg-avail">Available</span>
              <span className="fp-leg fp-leg-resv">Reserved</span>
              {editMode ? (
                <span className="fp-leg-hint">Drag to move · click to edit · ✕ to remove</span>
              ) : (
                <span className="fp-leg-hint">Hover for guest details</span>
              )}
            </div>
            ...
            <div className="fp-wrap" ref={wrapRef}>
              ...
              >
                ...
                <text x={SVG_W - 75} y={58} textAnchor="middle" fill="rgba(212,37,37,0.5)" style={{ fontSize: 9, fontFamily: "var(--sans)", letterSpacing: "0.06em", textTransform: "uppercase" }}>Charcoal Grill</text>
              ...
              {tooltip && !editMode && (
                ...
                >
                  <p className="fp-tooltip-title">Table {tooltip.table.label}</p>
                  <p className="fp-tooltip-cap">{tooltip.table.capacity} seats · {tooltip.table.zone}</p>
                  {tooltip.res ? (
                    <>
                      <div className="fp-tooltip-divider" />
                      <p className="fp-tooltip-name">{tooltip.res.user_name}</p>
                      <p className="fp-tooltip-meta">{tooltip.res.time} · {tooltip.res.party_size} {tooltip.res.party_size === 1 ? "person" : "people"}</p>
                      ...
                      <span className={`badge badge-${tooltip.res.payment_status === "paid" ? "green" : "amber"}`} style={{ fontSize: "0.7rem", marginTop: "0.35rem", display: "inline-block" }}>
                        {tooltip.res.payment_status}
                      </span>
                      <p className="fp-tooltip-hint">Click for full details</p>
                    </>
                  ) : (
                    <p className="fp-tooltip-avail">Available today</p>
                  )}
                </div>
              )}
            </div>
      ...
      {detailRes && detailTable && (
        <div className="modal-backdrop" onClick={() => { setDetailRes(null); setDetailTable(null); }}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            <h2 className="modal-title">Table {detailTable.label} · Guest</h2>
            <dl className="ticket-rows" style={{ marginTop: "1rem" }}>
              <div><dt>CCM Code</dt><dd style={{ color: "var(--amber)", fontFamily: "var(--mono)", fontSize: "0.88rem" }}>{detailRes.ccm_code ?? ""}</dd></div>
              <div><dt>Name</dt><dd>{detailRes.user_name}</dd></div>
              <div><dt>Email</dt><dd style={{ fontSize: "0.85rem" }}>{detailRes.user_email}</dd></div>
              <div><dt>Phone</dt><dd>{detailRes.phone}</dd></div>
              <div><dt>Time</dt><dd>{detailRes.date} at {detailRes.time}</dd></div>
              <div><dt>Party</dt><dd>{detailRes.party_size} {detailRes.party_size === 1 ? "person" : "people"}</dd></div>
              <div><dt>Table</dt><dd>{detailTable.label} · {detailTable.capacity} seats · {detailTable.zone}</dd></div>
              <div><dt>Payment</dt><dd><span className={`badge badge-${detailRes.payment_status === "paid" ? "green" : "amber"}`}>{detailRes.payment_status}</span></dd></div>
              {detailRes.note && <div><dt>Note</dt><dd style={{ color: "var(--text-muted)", fontStyle: "italic" }}>"{detailRes.note}"</dd></div>}
            </dl>
            <div className="modal-actions">
              <button className="btn btn-outline" onClick={() => { setDetailRes(null); setDetailTable(null); }}>Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editTarget && (
        <div className="modal-backdrop" onClick={() => setEditTarget(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            <h2 className="modal-title">Edit table {editTarget.label}</h2>
          ...
          <div className="modal-box" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="form" style={{ gap: "0.85rem" }}>
              <div className="form-row two">
                <label>
                  Label
                  <input type="text" placeholder="e.g. T9" value={addForm.label} onChange={(e) => setAddForm((f) => ({ ...f, label: e.target.value }))} />
                </label>
                <label>
                  Capacity
              ...
              <div className="form-row two">
                <label>Position X<input type="number" min={0} max={SVG_W} value={addForm.pos_x} onChange={(e) => setAddForm((f) => ({ ...f, pos_x: Number(e.target.value) }))} /></label>
                <label>Position Y<input type="number" min={0} max={SVG_H} value={addForm.pos_y} onChange={(e) => setAddForm((f) => ({ ...f, pos_y: Number(e.target.value) }))} /></label>
              </div>
            </div>
            <div className="modal-actions">
      ...
      {addFurnitureOpen && (
        <div className="modal-backdrop" onClick={() => setAddFurnitureOpen(false)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            <h2 className="modal-title">Add furniture</h2>
            <div className="fp-furniture-grid">
              {(Object.entries(FURNITURE_LABELS) as [FurnitureType, string][]).map(([type, label]) => {
            ...
            <div className="modal-actions">
              <button className="btn btn-outline" onClick={() => setAddFurnitureOpen(false)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteTarget && (
        <div className="modal-backdrop" onClick={() => setDeleteTarget(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            <h2 className="modal-title">Remove table {deleteTarget.label}?</h2>
            <p className="modal-body">This removes the table from the floor plan and makes it unavailable for new bookings.</p>
            <div className="modal-actions">
      ...
      {removeFurnitureId && (
        <div className="modal-backdrop" onClick={() => setRemoveFurnitureId(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="modal-actions">
              <button className="btn btn-outline" onClick={() => setRemoveFurnitureId(null)}>Cancel</button>
              <button className="btn btn-danger" onClick={() => removeFurniture(removeFurnitureId)}>Remove</button>
            </div>
          </div>
        </div>
      )}
