...
import { parseSiteConfig, type FeatureName, type HomeSection, type SiteConfig } from "~/lib/siteConfig";
...
function toggle<T extends Record<string, boolean>>(value: T, key: keyof T): T {
...
export function SiteControl() {
  ...
  useEffect(() => {
    if (settings.data) {
      const raw = (settings.data as typeof settings.data & { site_config_json?: string }).site_config_json;
      setDraft(parseSiteConfig(raw));
    }
  }, [settings.data]);
  ...
  const dirty = useMemo(() => {
    ...
    const raw = (settings.data as typeof settings.data & { site_config_json?: string }).site_config_json;
    return JSON.stringify(parseSiteConfig(raw)) !== JSON.stringify(draft);
  }, [draft, settings.data]);

  const dependencyWarning =
    draft && !draft.features.customerAccounts && (draft.features.booking || draft.features.loyalty)
      ? "Customer accounts are required by the current booking/loyalty flows. Keep Customer accounts on, or disable those dependent features too."
      : null;

  if (!draft) {
    return (
      <DeskPage title="Site control" lead="Control what customers can see and what the restaurant is accepting right now.">
        <Loaded resource={settings} skeletonHeight="16rem">{() => null}</Loaded>
      </DeskPage>
    );
  }
  ...
  async function save() {
    setBusy(true);
    try {
      await api.desk.settings.update({ site_config_json: JSON.stringify(draft) } as never);
      settings.reload();
      venue.refresh();
  ...
  function setMode(mode: SiteConfig["business"]["mode"]) {
    setDraft({ ...draft, business: { ...draft.business, mode } });
  }
      ...
      <div className="stack stack--loose">
        ...
        <section className="card stack site-control__grid">
          <div className="site-control__section">
            <div className="section-head">
              <h2 className="display display--md">Live restaurant state</h2>
              <p className="fine muted">This changes the status customers see across the preview site.</p>
            </div>
            <div className="site-control__mode">
              {(["open", "busy", "closed"] as const).map((mode) => (
                <button key={mode} type="button" data-active={draft.business.mode === mode} onClick={() => setMode(mode)}>
                  {mode === "open" ? "Open" : mode === "busy" ? "Busy right now" : "Closed"}
                </button>
              ))}
            </div>
            <TextAreaField
              label="Customer message"
              hint="Shown with the live status. Keep it short and human."
              rows={3}
              value={draft.business.message.en}
              onChange={(event) => setDraft({ ...draft, business: { ...draft.business, message: { ...draft.business.message, en: event.target.value } } })}
            />
            <TextAreaField
              label="Message en français"
              rows={3}
              value={draft.business.message.fr}
              onChange={(event) => setDraft({ ...draft, business: { ...draft.business, message: { ...draft.business.message, fr: event.target.value } } })}
            />
          </div>
        </section>
        ...
        <div className="site-control__grid">
          <section className="card stack site-control__section site-control__section--half">
            <div className="section-head">
              <h2 className="display display--md">Customer features</h2>
              <p className="fine muted">Turn a feature off and the customer navigation, route and relevant calls to action disappear or show a friendly unavailable state.</p>
            </div>
            {featureRows.map((item) => (
              <Switch
                key={item.key}
                checked={draft.features[item.key]}
                label={<span className="site-control__label"><strong>{item.label}</strong><span className="site-control__hint">{item.hint}</span></span>}
          ...
          <section className="card stack site-control__section site-control__section--half">
            <div className="section-head">
              <h2 className="display display--md">Homepage sections</h2>
              <p className="fine muted">Keep the homepage focused. These switches only affect customer-facing sections; they never delete content.</p>
            </div>
            {homeRows.map((item) => (
              <Switch
                key={item.key}
                checked={draft.homepage[item.key]}
                label={<span className="site-control__label"><strong>{item.label}</strong><span className="site-control__hint">{item.hint}</span></span>}
                onChange={() => setDraft({ ...draft, homepage: toggle(draft.homepage, item.key) })}
              />
            ))}
          </section>
        </div>
        ...
        <div className="site-control__grid">
          <section className="card stack site-control__section site-control__section--half">
            <div className="section-head">
              <h2 className="display display--md">Service controls</h2>
              <p className="fine muted">Pausing a service keeps the rest of the site usable and gives customers a clear explanation instead of a broken screen.</p>
            </div>
            <ServiceSwitch label="Takeaway" feature="ordering" mode={draft.services.ordering.mode} setMode={(mode) => setDraft({ ...draft, services: { ...draft.services, ordering: { ...draft.services.ordering, mode } } })} />
            <ServiceSwitch label="Table booking" feature="booking" mode={draft.services.booking.mode} setMode={(mode) => setDraft({ ...draft, services: { ...draft.services, booking: { ...draft.services.booking, mode } } })} />
            <ServiceSwitch label="Waitlist" feature="waitlist" mode={draft.services.waitlist.mode} setMode={(mode) => setDraft({ ...draft, services: { ...draft.services, waitlist: { ...draft.services.waitlist, mode } } })} />
          </section>

          <section className="card stack site-control__section site-control__section--half">
            <div className="section-head">
              <h2 className="display display--md">Payments</h2>
              <p className="fine muted">CCM accepts only the two configured mobile-money methods below. There is no card payment option.</p>
            </div>
            ...
            {!draft.payments.mtn && !draft.payments.orange ? <Notice tone="warn">No payment method is enabled. Customers will not be able to complete paid bookings or takeaway orders.</Notice> : null}
          </section>
        </div>

        <section className="card stack">
          <div className="section-head">
            <h2 className="display display--md">Support</h2>
            <p className="fine muted">Control where the floating customer support entry point appears and how it describes availability.</p>
          </div>
        ...
        <section className="card stack">
          <div className="section-head">
            <h2 className="display display--md">Languages</h2>
            <p className="fine muted">English and French are the customer languages. The selection is remembered on the device and the document language updates with it for accessibility.</p>
          </div>
          ...
          <div className="row row--wrap">
            {(["en", "fr"] as const).map((locale) => (
              <button key={locale} type="button" className={`btn ${draft.defaultLocale === locale ? "btn--primary" : "btn--ghost"}`} disabled={!draft.locales[locale]} onClick={() => setDraft({ ...draft, defaultLocale: locale })}>
                Default: {locale.toUpperCase()}
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="card stack">
          <div className="section-head">
            <h2 className="display display--md">Announcement bar</h2>
            <p className="fine muted">One short, customer-visible message across the site.</p>
          </div>
          ...
          <select className="select" value={draft.announcement.tone} onChange={(event) => setDraft({ ...draft, announcement: { ...draft.announcement, tone: event.target.value as SiteConfig["announcement"]["tone"] } })}>
        ...
        <div className="row row--between">
          <span className={dirty ? "site-control__status" : "fine faint"}>{dirty ? "Unsaved changes" : "All customer controls are saved."}</span>
          <Button tone="primary" busy={busy} disabled={!dirty} onClick={() => void save()}>Save site control</Button>
        </div>
      </div>
    </DeskPage>
  );
...
function ServiceSwitch({
  ...
  mode: "open" | "paused";
  setMode: (mode: "open" | "paused") => void;
}) {
  return (
    <Switch
      checked={mode === "open"}
      label={<span className="site-control__label"><strong>{label}</strong><span className="site-control__hint">{mode === "open" ? "Accepting customers" : "Paused for customers"}</span></span>}
      onChange={(checked) => setMode(checked ? "open" : "paused")}
    />
  );
}
