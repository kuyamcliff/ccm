...
/**
 * A section that opens on tap instead of costing its full height whether or
 * not anybody is looking at it right now — eight of these stacked, all
 * expanded, is the console's own worst case for "too big to read at a
 * glance". Live status stays open by default; the rest is one tap away.
 */
function Disclosure({
  title,
  lead,
  ...
  return (
    <details className={`card site-control__disclosure${className ? ` ${className}` : ""}`} open={defaultOpen}>
      <summary>
        <span className="site-control__disclosure-head">
          <strong className="display display--md">{title}</strong>
          <span className="fine muted">{lead}</span>
        </span>
        <Icon name="chevron-down" size={18} />
      </summary>
      <div className="stack site-control__disclosure-body">{children}</div>
    </details>
  );
}
...
export function SiteControl() {
  ...
  return (
    <DeskPage title="Site control" lead="Control customer-facing features, homepage sections, service availability, support and live restaurant state.">
      ...
      <div className="stack site-control">
        {dependencyWarning ? <Notice tone="warn" title="Feature dependency">{dependencyWarning}</Notice> : null}
        <Disclosure title="Live restaurant state" lead="This changes the status customers see across the preview site." defaultOpen><div className="site-control__mode">{(["open", "busy", "closed"] as const).map((mode) => <button key={mode} type="button" data-active={draft.business.mode === mode} onClick={() => setMode(mode)}>{mode === "open" ? "Open" : mode === "busy" ? "Busy right now" : "Closed"}</button>)}</div><TextAreaField label="Customer message" hint="Shown with the live status. Keep it short and human." rows={3} value={draft.business.message.en} onChange={(event) => setDraft({ ...draft, business: { ...draft.business, message: { ...draft.business.message, en: event.target.value } } })} /><TextAreaField label="Message en français" rows={3} value={draft.business.message.fr} onChange={(event) => setDraft({ ...draft, business: { ...draft.business, message: { ...draft.business.message, fr: event.target.value } } })} /></Disclosure>
        <div className="site-control__grid"><Disclosure title="Customer features" lead="Turn a feature off and the customer navigation, route and relevant calls to action disappear or show a friendly unavailable state." className="site-control__section--half"><p className="fine faint">A schedule narrows when a feature is on. It can only take away: a feature you switch off stays off inside its window.</p>{featureRows.map((item) => <FeatureRow key={item.key} item={item} draft={draft} setDraft={setDraft} />)}</Disclosure><Disclosure title="Homepage sections" lead="Choose exactly which content blocks customers see on the homepage." className="site-control__section--half">{homeRows.map((item) => <Switch key={item.key} checked={draft.homepage[item.key]} label={<span className="site-control__label"><strong>{item.label}</strong><span className="site-control__hint">{item.hint}</span></span>} onChange={() => setDraft({ ...draft, homepage: toggle(draft.homepage, item.key) })} />)}</Disclosure></div>
        <div className="site-control__grid"><Disclosure title="Service controls" lead="Pausing a service keeps the rest of the site usable and gives customers a clear explanation." className="site-control__section--half"><ServiceSwitch label="Takeaway" mode={draft.services.ordering.mode} setMode={(mode) => setDraft({ ...draft, services: { ...draft.services, ordering: { ...draft.services.ordering, mode } } })} /><ServiceSwitch label="Table booking" mode={draft.services.booking.mode} setMode={(mode) => setDraft({ ...draft, services: { ...draft.services, booking: { ...draft.services.booking, mode } } })} /><ServiceSwitch label="Waitlist" mode={draft.services.waitlist.mode} setMode={(mode) => setDraft({ ...draft, services: { ...draft.services, waitlist: { ...draft.services.waitlist, mode } } })} /></Disclosure><Disclosure title="Payments" lead="CCM accepts only MTN Mobile Money and Orange Money. There is no card payment option." className="site-control__section--half"><Switch checked={draft.payments.mtn} label="MTN Mobile Money" onChange={() => setDraft({ ...draft, payments: { ...draft.payments, mtn: !draft.payments.mtn } })} /><Switch checked={draft.payments.orange} label="Orange Money" onChange={() => setDraft({ ...draft, payments: { ...draft.payments, orange: !draft.payments.orange } })} />{!draft.payments.mtn && !draft.payments.orange ? <Notice tone="warn">No payment method is enabled. Customers cannot complete paid bookings or takeaway orders.</Notice> : null}</Disclosure></div>
        <Disclosure title="Support" lead="Control where customer support appears and how it describes availability."><Switch checked={draft.support.enabled} label="Enable customer support chat" onChange={() => setDraft({ ...draft, support: { ...draft.support, enabled: !draft.support.enabled } })} /><Switch checked={draft.support.staffed} label="Show support as staffed" onChange={() => setDraft({ ...draft, support: { ...draft.support, staffed: !draft.support.staffed } })} /><Switch checked={draft.support.whatsapp} label="Show WhatsApp contact" onChange={() => setDraft({ ...draft, support: { ...draft.support, whatsapp: !draft.support.whatsapp } })} /><Switch checked={draft.support.phone} label="Show phone contact" onChange={() => setDraft({ ...draft, support: { ...draft.support, phone: !draft.support.phone } })} /><label className="field"><span className="field__label">Typical support reply, minutes</span><input className="input" type="number" min={1} max={1440} value={draft.support.responseMinutes} onChange={(event) => setDraft({ ...draft, support: { ...draft.support, responseMinutes: Math.max(1, Math.min(1440, Number(event.target.value) || 1)) } })} /></label><TextAreaField label="After-hours message (English)" rows={2} value={draft.support.afterHoursMessage.en} onChange={(event) => setDraft({ ...draft, support: { ...draft.support, afterHoursMessage: { ...draft.support.afterHoursMessage, en: event.target.value } } })} /><TextAreaField label="After-hours message (French)" rows={2} value={draft.support.afterHoursMessage.fr} onChange={(event) => setDraft({ ...draft, support: { ...draft.support, afterHoursMessage: { ...draft.support.afterHoursMessage, fr: event.target.value } } })} /></Disclosure>
        <Disclosure title="Languages" lead="English and French are the customer languages."><Switch checked={draft.locales.en} label="English" onChange={() => setDraft({ ...draft, locales: { ...draft.locales, en: !draft.locales.en } })} /><Switch checked={draft.locales.fr} label="French" onChange={() => setDraft({ ...draft, locales: { ...draft.locales, fr: !draft.locales.fr } })} /><div className="row row--wrap">{(["en", "fr"] as const).map((locale) => <button key={locale} type="button" className={`btn ${draft.defaultLocale === locale ? "btn--primary" : "btn--ghost"}`} disabled={!draft.locales[locale]} onClick={() => setDraft({ ...draft, defaultLocale: locale })}>Default: {locale.toUpperCase()}</button>)}</div></Disclosure>
        <Disclosure title="Announcement bar" lead="One short customer-visible message across the site."><Switch checked={draft.announcement.enabled} label="Show announcement" onChange={() => setDraft({ ...draft, announcement: { ...draft.announcement, enabled: !draft.announcement.enabled } })} /><select className="select" value={draft.announcement.tone} onChange={(event) => setDraft({ ...draft, announcement: { ...draft.announcement, tone: event.target.value as SiteConfig["announcement"]["tone"] } })}><option value="info">Info</option><option value="good">Good news</option><option value="warn">Important</option></select><TextAreaField label="English" rows={2} value={draft.announcement.message.en} onChange={(event) => setDraft({ ...draft, announcement: { ...draft.announcement, message: { ...draft.announcement.message, en: event.target.value } } })} /><TextAreaField label="French" rows={2} value={draft.announcement.message.fr} onChange={(event) => setDraft({ ...draft, announcement: { ...draft.announcement, message: { ...draft.announcement.message, fr: event.target.value } } })} /></Disclosure>
        <Disclosure title="Maintenance" lead="Closes the site to customers while you work on it. You and your staff keep full access, including this page, so you can always turn it back off." defaultOpen={draft.maintenance.enabled}><Switch checked={draft.maintenance.enabled} label={<span className="site-control__label"><strong>Close the site to customers</strong><span className="site-control__hint">Customers see your message and the phone number. Signing in stays open.</span></span>} onChange={() => setDraft({ ...draft, maintenance: { ...draft.maintenance, enabled: !draft.maintenance.enabled } })} />{draft.maintenance.enabled ? <Notice tone="warn" title="The site is closed to customers">Nobody can book a table, order takeaway or join the queue while this is on. Staff are unaffected.</Notice> : null}<TextAreaField label="What customers see (English)" hint="Say what is happening and roughly how long. Nothing has gone wrong, so do not make it read like an error." rows={3} value={draft.maintenance.message.en} onChange={(event) => setDraft({ ...draft, maintenance: { ...draft.maintenance, message: { ...draft.maintenance.message, en: event.target.value } } })} /><TextAreaField label="What customers see (French)" rows={3} value={draft.maintenance.message.fr} onChange={(event) => setDraft({ ...draft, maintenance: { ...draft.maintenance, message: { ...draft.maintenance.message, fr: event.target.value } } })} /></Disclosure>
