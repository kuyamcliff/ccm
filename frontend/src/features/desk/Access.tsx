...
export function Access() {
  ...
  async function applyTemplate(staffer: StaffAccess, templateIndex: number) {
    ...
    try {
      ...
      for (const scope of Object.keys(staffer.scopes) as AdminScope[]) { const next = wanted.has(scope); if (staffer.scopes[scope] !== next) await api.desk.access.setScope(staffer.id, scope, next); }
      access.set((current) => {
        if (!current) return { scopes: [], staff: [] };
        return { ...current, staff: current.staff.map((s) => s.id === staffer.id ? { ...s, scopes: Object.fromEntries((Object.keys(s.scopes) as AdminScope[]).map((scope) => [scope, wanted.has(scope)])) as StaffAccess["scopes"] } : s) };
      });
  ...
  return <DeskPage title="Access" lead="Decide who can enter the Desk and what each plain admin can reach. The owner controls this page.">
    {confirmElement}
    ...
    <Loaded resource={access}>{(data) => data.staff.length === 0 ? <Nothing>Nobody has staff access yet.</Nothing> : <TableWrap>
      <thead><tr><th>Who</th><th>Role</th><th>Access</th><th>Joined</th><th /></tr></thead>
      <tbody>{data.staff.map((row) => { const lockedCount = Object.values(row.scopes).filter((v) => !v).length; return <tr key={row.id}>
    ...
    <PermissionsSheet staffer={editing} scopes={access.data?.scopes ?? []} onClose={() => setEditing(null)} onChanged={(id, scope, granted) => { access.set((current) => current ? { ...current, staff: current.staff.map((s) => s.id === id ? { ...s, scopes: { ...s.scopes, [scope]: granted } } : s) } : { scopes: [], staff: [] }); setEditing((cur) => cur && cur.id === id ? { ...cur, scopes: { ...cur.scopes, [scope]: granted } } : cur); }} />
  </DeskPage>;
}
...
function PermissionsSheet({ staffer, scopes, onClose, onChanged }: { staffer: StaffAccess | null; scopes: { key: AdminScope; label: string; hint: string }[]; onClose: () => void; onChanged: (id: number, scope: AdminScope, granted: boolean) => void; }) {
  const toast = useToast(); const [busy, setBusy] = useState<AdminScope | null>(null); const lockedCount = useMemo(() => staffer ? Object.values(staffer.scopes).filter((v) => !v).length : 0, [staffer]);
  return <Sheet open={staffer !== null} onClose={onClose} title={staffer ? `${staffer.name}'s access` : ""} description={staffer ? lockedCount === 0 ? "Every page is switched on." : `${lockedCount} page${lockedCount === 1 ? "" : "s"} locked.` : undefined}>
    {staffer ? <div className="stack"><Notice tone="info">These are page-level controls today. The server remains authoritative even when a page is hidden from the Desk.</Notice>{scopes.map((scope) => <Switch key={scope.key} label={<span>{scope.label}<span className="fine faint" style={{ display: "block" }}>{scope.hint}</span></span>} checked={staffer.scopes[scope.key] ?? true} disabled={busy === scope.key} onChange={async (next) => { setBusy(scope.key); try { await api.desk.access.setScope(staffer.id, scope.key, next); onChanged(staffer.id, scope.key, next); } catch (err) { toast.failed(err); } finally { setBusy(null); } }} />)}</div> : null}
  </Sheet>;
}
