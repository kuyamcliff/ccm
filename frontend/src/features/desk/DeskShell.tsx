  ...
  const visibleGroups = useMemo(() => {
    ...
    return GROUPS.map((group) => ({
      ...
      items: group.items.filter((item) => {
        const visible = (!item.topOwner || isTopOwner) && (!item.owner || isOwner) && (!item.scope || can(item.scope));
        return visible && (!normalized || `${group.title} ${item.label}`.toLowerCase().includes(normalized));
      }),
