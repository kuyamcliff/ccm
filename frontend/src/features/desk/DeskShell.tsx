...
export function DeskShell() {
  ...
  return <div className="desk"><a className="skip-link" href="#desk-main">Skip to content</a><header className="desk__bar"><IconButton name={drawer ? "close" : "menu"} label={drawer ? "Close menu" : "Open menu"} className="desk__burger" onClick={() => setDrawer((open) => !open)} /><Link to="/desk" className="brand"><img src="/mark.svg" alt="" className="brand__mark" width={28} height={28} /><span className="brand__name">Desk<span>.</span></span></Link><div className="push row"><Link to="/" className="btn btn--quiet btn--sm"><Icon name="external" size={16} />The site</Link><Link to="/site-control" className="btn btn--quiet btn--sm"><Icon name="settings" size={16} />Site control</Link><span className="desk__who"><Avatar name={user?.name ?? ""} /><span className="fine">{user?.name}<span className="faint"> {isTopOwner ? "owner" : isOwner ? "super admin" : "staff"}</span></span></span><IconButton name="logout" label="Sign out" onClick={() => void signOut()} /></div></header><div className="desk__body"><nav className={`desk__rail${drawer ? " desk__rail--open" : ""}`} aria-label="Console">{GROUPS.map((group) => { const items = group.items.filter((item) => (!item.topOwner || isTopOwner) && (!item.owner || isOwner) && (!item.scope || can(item.scope))); if (items.length === 0) return null; return <div key={group.title} className="desk__group"><p className="desk__grouptitle">{group.title}</p>{items.map((item) => <NavLink key={item.to} to={item.to} end={item.to === "/desk"} className="desk__link"><Icon name={item.icon} size={18} />{item.label}</NavLink>)}</div>; })}</nav>{drawer ? <button type="button" className="desk__scrim" aria-label="Close menu" onClick={() => setDrawer(false)} /> : null}<main id="desk-main" className="desk__main" tabIndex={-1}><Outlet /></main></div></div>;
}
