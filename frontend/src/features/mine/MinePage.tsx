...
function Orders({ orders }: { orders: Resource<TakeawayOrder[]> }) {
  ...
  const orderAgain = (order: TakeawayOrder) => {
    ...
    if (menu.data) {
      ...
      clear();
      for (const line of available) add(line.id, line.qty);
      toast.done(locale === "fr" ? "Votre commande précédente est de retour dans le panier." : "Your previous order is back in the basket.");
      navigate("/order");
      return;
    }
