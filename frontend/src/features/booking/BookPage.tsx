...
export function BookPage() {
  ...
  if (!user) {
    return <div className="page section"><div className="section-head"><hr className="heat-rule" /><h1 className="display display--xl">Book a table</h1></div><div className="stack stack--loose" style={{ maxWidth: "32rem" }}><p className="lead">Booking needs an account, so your table, your code and your receipt stay in one place and you can change or cancel without calling anybody.</p><div className="actions"><LinkButton to="/join" tone="primary" size="lg">Create an account</LinkButton><LinkButton to="/signin" tone="ghost" size="lg">I already have one</LinkButton></div><p className="fine faint">In a hurry? <Link to="/order">Order takeaway</Link> instead, or <Link to="/waitlist">join the queue</Link> if you are already outside.</p></div></div>;
  }

  if (paid) {
    return <div className="page section stack stack--loose"><div className="section-head"><hr className="heat-rule" /><h1 className="display display--xl">Table held</h1><p className="lead">Show this at the door. It is also in Mine, so you do not need to keep this page open.</p></div><BookingPass booking={{ ...paid, status: "confirmed", payment_status: "paid" }} /><div className="actions"><LinkButton to="/mine" tone="primary" iconEnd="arrow-right">See my bookings</LinkButton><Button tone="ghost" onClick={() => { setPaid(null); setPending(null); setTime(null); setTableId(null); setBasket({}); setParams({}, { replace: true }); }}>Book another</Button></div></div>;
  }

  const forward = step === "review" ? <Button tone="primary" size="lg" icon="wallet" busy={create.busy} onClick={submit}>Pay {money(dueNow)} FCFA</Button> : <Button tone="primary" size="lg" iconEnd="arrow-right" disabled={(step === "when" && !time) || (step === "who" && !phoneReady)} onClick={() => go(ORDER[index + 1]!)}>Continue</Button>;
  const recap = step === "when" ? (time ? `${dayLabel(date)}, ${timeLabel(time)}` : "Pick a day and a time") : step === "who" ? (phoneReady ? `${party === 1 ? "Just me" : `${party} people`}, ${dayLabel(date)}` : "We need a phone number") : step === "where" ? (chosenTable ? `Table ${chosenTable.label}, seats ${chosenTable.capacity}` : "Any table") : `${money(dueNow)} FCFA to pay now`;

  return <div className="booking">
    <header className="booking__head"><div className="page booking__head-in"><button type="button" className="booking__back" onClick={back} aria-label="Back"><Icon name="arrow-left" size={20} /></button><span className="label">Step {index + 1} of {ORDER.length}</span><ol className="booking__ticks">{ORDER.map((name, i) => <li key={name}><button type="button" data-state={i < index ? "done" : i === index ? "now" : "todo"} disabled={i > index} aria-label={`Step ${i + 1}, ${TITLES[name]}`} aria-current={i === index ? "step" : undefined} onClick={() => go(name)} /></li>)}</ol></div></header>
    ...
    {pending ? <MomoDialog
      ...
      onClose={() => { setPending(null); toast.say("Your table is held for 30 minutes while you pay."); navigate("/mine"); }}
