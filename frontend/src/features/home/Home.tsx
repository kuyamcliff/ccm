...
function Hero({ images, lowestPrice }: { images: string[]; lowestPrice: number | null }) {
  ...
  const statusLabel = siteConfig.business.mode === "closed" ? t("closed") : siteConfig.business.mode === "busy" ? t("busy") : t("openNow");
  return <section className="hero"><div className={`hero__media${images.length === 0 ? " hero__media--bare" : ""}`}><div className="hero__frames" aria-hidden="true">{images.slice(0, 3).map((src, index) => <div key={src} className={`hero__frame hero__frame--${index + 1}`}><Photo src={src} alt="" eager={index === 0} /></div>)}</div><p className="hero__eyebrow label"><span className="hero__coal" aria-hidden="true" />{address}</p></div><div className="page hero__inner"><h1 className="display display--hero hero__title">{locale === "fr" ? "Frais, grillé au feu." : "Fresh off the fire."}<br /><span className="hero__hot">{locale === "fr" ? "À Buea." : "In Buea."}</span></h1><p className="hero__blurb">{locale === "fr" ? "Poulet, porc et chèvre grillés à la commande, avec vos accompagnements préférés. Réservez ou commandez directement depuis votre téléphone." : "Chicken, pork and goat grilled fresh over the fire, with the sides you actually want. Book or order straight from your phone."}</p><div className="hero__actions"><LinkButton to="/menu" tone="primary" icon="list">{t("seeMenu")}</LinkButton>{orderingOpen ? <LinkButton to="/order" tone="ghost" icon="bag">{t("orderNow")}</LinkButton> : bookingOpen ? <LinkButton to="/book" tone="ghost" icon="calendar">{t("book")}</LinkButton> : null}</div><dl className="hero__facts"><div><dt className="label">{statusLabel}</dt><dd>{siteConfig.business.mode === "open" ? hours : siteConfig.business.message[locale]}</dd></div><div><dt className="label">{locale === "fr" ? "À partir de" : "Plates from"}</dt><dd className="mono">{lowestPrice !== null ? `${money(lowestPrice)} FCFA` : "—"}</dd></div><div><dt className="label">{t("call")}</dt><dd>{phoneHref ? <a href={phoneHref}>{phoneLabel(phone ?? "")}</a> : (locale === "fr" ? "Au comptoir" : "At the counter")}</dd></div></dl></div></section>;
}
...
function OfferHighlight() {
  const { data, loading } = useResource(() => api.site.offers(), []); const { locale } = useLocale();
...
function GalleryHighlight() {
  const { data, loading } = useResource(() => api.site.gallery(), []); const { locale } = useLocale(); const photos = (data ?? []).filter((item) => item.is_approved === 1).slice(0, 3);
...
function Ways() {
  const { depositFcfa, siteConfig } = useVenue(); const { locale } = useLocale();
  const items = [
...
export function Home() {
  const { user, ready } = useSession(); const { data } = useResource(() => api.site.highlights(), []); const { siteConfig } = useVenue(); const images = (data?.topItems ?? []).flatMap((item) => (item.image_url ? [item.image_url] : [])); const prices = (data?.topItems ?? []).map((item) => item.price_fcfa).filter((v): v is number => typeof v === "number" && v > 0); const lowestPrice = prices.length ? Math.min(...prices) : null; const signedIn = ready && user !== null;
  return <>{siteConfig.homepage.hero ? (signedIn ? <YourStuff name={user.name} /> : <Hero images={images} lowestPrice={lowestPrice} />) : null}{siteConfig.homepage.featured ? <Favourites /> : null}{siteConfig.homepage.offer && siteConfig.features.offers ? <OfferHighlight /> : null}{siteConfig.homepage.ways ? <Ways /> : null}{siteConfig.homepage.gallery && siteConfig.features.gallery ? <GalleryHighlight /> : null}{siteConfig.homepage.accountCta && siteConfig.features.customerAccounts && !signedIn ? <WhyAnAccount /> : null}{siteConfig.homepage.reviews && siteConfig.features.reviews ? <WordOfMouth /> : null}{siteConfig.homepage.location ? <FindUs /> : null}</>;
}
